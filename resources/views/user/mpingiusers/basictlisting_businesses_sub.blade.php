                        ...
                        <div class="row align-items-center justify-content-center">
                            <div class="" id="" style="">
                                ...
                                <p id="servicesSection" class="mt-3"
                                    ...
                                    <a href="{{ route('user.basiclistingbusinessesID', ['id'=> encrypt($s->id)]) }}" class="" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $s->service_name }} (0)">
                            ...
                            <div class="row">
                                @foreach ($servicesid as $row)
                                    ...
                                    <div class="col-11">
                                        ...
                                        <i class="{{ $row->service_image }}" style="font-size: 1.3rem"
                                             height="40"
                                            width="40"></i>
                                        {{ $row->service_name }}
                                        <font color="#ff7519">&nbsp;&nbsp;» </font>
                                        <a href="#" class="hrefCss3s" style="color:rgb(255, 117, 25)">@foreach ($subservice_name as $w){{ $w->sub_service_1_name }}  @endforeach</a>
                               ...
                               <div class="row forcard">
                                 <div class="mb-3 col-md-6">
                                     <div class="card card_3">
                                            <div class="card-body">
                                             @foreach ($subservice_name as $c)
                                                <h1 class="h1Css">
                                                    <img class="imgCss"
                                                        src="{{ '/storage/assets/images/avatar_nzuzi.png' }}"
                                                        alt="" width="80" height="80"
                                                        class="img_icons_3 zoomIn animated"> {{ $c->sub_service_1_name }}
                                                        <span></span>
                                                         @foreach($countsubservice as $s)
                                                        {{-- {{ $ss->id }} --}}
                                                           @if($s->service_id == $c->id)
                                                            <span class="badge_3">({{ $s->subservice_cnt }}) </span>
                                                           @else
                                                           {{-- <span style="font: 10pt/130% Helvetica, Arial, sans-serif;">(0)</span> --}}
                                                           @endif
                                                          @endforeach
                                                 ...
                                                 @endforeach --}}
                                                 @foreach ($subservice_sub_two as $sso)
                                                 <div class="mb-2">
                                                    <a href="{{ route('user.basiclistingbusinessessubservicesid', ['sub_service_id'=> encrypt($sso->sub_service_id), 'service_id'=> encrypt($sso->service_id)]) }}" class="link-secondary sidemenu_a2">
                                                    <span><span class="TextCss accordion"  style="margin-left:10%;"> {{ $sso->sub_service_2_name }}</span>
                                                     &nbsp;


                                                 @foreach($countsubservice2 as $s)

                                                     @if($s->sub_service_1_id == $sso->sub_service_one_id)
                                                     <span class="badge bg-light text-dark">
                                                                  <span class="badge rounded-pill bg-light text-dark">    <span class="badge_3">({{ $s->subservice_cnt2 }}) </span></span>
                                                        </span>
                                                        @else
                                                        {{-- <span class="badge bg-light text-dark">
                                                            <span class="badge rounded-pill bg-light text-dark">    <span class="badge_3">(0) </span></span>
                                                        </span> --}}

                                                        @endif
                                                        @endforeach
                                                          <b
                                                         class="hoverClass accordion" style="float: right;font-size:1rem">+</b></span>
                                   ...
                                   @foreach ($subservice_name as $c)
                                   @endforeach
                                   @foreach ($subservice_name as $v)
                                   <div class="col-md-6">
                                       <a href="{{ route('user.basiclistingbusinessessubservicesform', ['id'=> encrypt($row->id), 'msid'=>encrypt($c->sub_service_id )]) }}" class="removeU" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $c->sub_service_1_name }}">
                                           <div class="mb-1">
                                               <i class="fas fa-bullseye" style="font-size:8px"></i>
                                              {{ \Illuminate\Support\Str::limit($v->sub_service_1_name, 16) }}
                                          @foreach($countsubservice as $s)
                                           {{-- {{ $ss->id }} --}}
                                              @if($s->service_id == $v->id)
                                               <span class="badge_3">({{ $s->subservice_cnt }}) </span>
                                              @else
                                              {{-- <span style="font: 10pt/130% Helvetica, Arial, sans-serif;">(0)</span> --}}
                                              @endif
